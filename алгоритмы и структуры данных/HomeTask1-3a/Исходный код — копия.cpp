...
         Initialization of arrays*/
	struct Block *blocks = static_cast<struct Block*>(calloc(number_of_blocks, sizeof(struct Block)));
    if (!blocks)
        return -1;
    /* In the array number_of_incoming_blocks is stored number of blocks that can be invested in the i-th block*/
	int *number_of_incoming_blocks = static_cast<int*>(calloc(number_of_blocks, sizeof(int)));
